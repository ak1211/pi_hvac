...
import InfraredRemoteCode (Baseband(..), Bit(..), Count(..), InfraredCodeFrame(..), InfraredHexString, decodePhase1, fromMilliseconds, infraredCodeTextParser, toIrCodeFrames, toMilliseconds)
...
inputIRCode3 = ""
...
inputIRCode4 = ""
...
expectIRCodeFormat4 =
  ...
  in
    FormatAEHA
      { octets:
          [ unsafePartial $ fromJust $ NEA.fromArray a
...
main = do
  ...
  --
  let
    count = fromMilliseconds (Milliseconds 3.2)
  log $ show count
  assert' ("expected: 123") (Count 123 == count)
  let
    msec = toMilliseconds count
  log $ show msec
  let
    count' = fromMilliseconds msec
  log $ show count'
  assert' ("expected: 123") (Count 123 == count')
  --
  log ""
  ...
  let
    value = decodePhase1 (Baseband tokens)
  let
    expected =
      [ [ { off: Count 60, on: Count 30 }, { off: Count 30, on: Count 30 }, { off: Count 384, on: Count 30 } ]
