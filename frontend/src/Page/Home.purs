...
-- |
type NonEmptyValues
  = NEA.NonEmptyArray Api.MeasEnvironment
...
render state =
  ...
  where
  gauge x = HH.div [ HP.class_ HB.colSm ] [ x ]
...
handleAction = case _ of
  ...
  Initialize -> do
    ...
    case maybeElem of
      Nothing -> pure unit
      Just elem -> void $ H.liftEffect $ Timer.setTimeout 60 (timerHandler elem)
    pure mempty
  Update -> do
    url <- getApiBaseURL
    ...
    val <-
      H.liftAff
        $ let
            param = { baseurl: url, limits: Nothing }
  ...
  timerHandler button = HTMLElement.click button
...
msgFailToAccess reason = Commons.toastItem "Error" "" reason

-- |
msgLastUpdatedAt :: forall p i. Milliseconds -> Api.MeasDateTime -> HH.HTML p i
msgLastUpdatedAt nowTime (Api.MeasDateTime measTime) = case Utils.asiaTokyoDateTime measTime of
  Nothing -> Commons.toastItem "Error" "" "有効な日付ではありませんでした"
  Just asiaTokyo ->
    Commons.snackbarItem
      $ String.joinWith " "
          [ "Measured at"
          , asiaTokyo.time <> ","
  ...
  boundedMinute :: Int
  boundedMinute = unsafePartial $ fromJust $ minimum [ 9999, minute ]
