...
initialState route =
  let
    qry = case route of
      Route.Plotdata Nothing -> initialQueryParams
...
handleAction = case _ of
  ...
  Initialize -> do
    { queryParams } <- H.get
    let
      newRoute = Route.Plotdata (Just queryParams)
  ...
  ChangedRoute route -> do
    ...
    let
      state = case route of
        Route.Plotdata Nothing -> curState { queryParams = initialQueryParams }
        Route.Plotdata (Just qry) -> curState { queryParams = qry }
        _ -> curState
    --
    url <- getApiBaseURL
    let
      param = { baseurl: url, limits: state.queryParams.limits }
    res <- H.liftAff $ Api.getApiV1Measurements param
    case res of
      Left a -> do
        H.put state
        H.liftEffect $ logShow a
      Right result -> H.put state { measValues = result.body }
  OnInputRecordsLimit strLimits -> do
    let
      newLimits = Int.fromString strLimits
    { queryParams } <- H.get
    let
      newQueryParams = queryParams { limits = newLimits }
...
chartLabels values = map (maybe "N/A" showDateTime <<< localDateTime <<< measDateTime) values
  where
...
measurementalTable measValues =
  ...
  tableRow (Api.MeasEnvironment v) =
    let
      datetime = Utils.asiaTokyoDateTime $ unwrap v.measured_at
    in
      HH.tr_
...
fromNumber = either (const "N/A") identity <<< FN.formatNumber "0.00"
