...
handleAction = case _ of
  ...
  OnClickI2CDetect -> do
    ...
    val <-
      H.liftAff
        $ let
            param = { baseurl: url, busnumber: Just 1 }
...
i2c = HH.span_ [ HH.text "I", HH.sup_ [ HH.text "2" ], HH.text "C" ]
...
i2cDevices = case _ of
  ...
  detectedDevAddresses addresses = map f addressTableRange
    where
    f x = fromMaybe (cellFiller x) $ Map.lookup x (devices addresses)

  cellFiller :: Int -> String
  cellFiller addr
    | Array.any (_ == addr) $ 0x03 .. 0x77 = "•"
    | otherwise = " "

  devices :: Array Int -> Map Int String
  devices xs = Map.fromFoldable $ map f xs
    where
...
tableHeading range = HH.thead_ [ HH.tr_ $ hd : tl ]
  where
  hd = HH.th_ [ HH.text "#" ]
...
tableBody range array2D = HH.tbody_ $ Array.zipWith tableRow range array2D

-- |
tableRow :: forall p i. Int -> Array String -> HH.HTML p i
tableRow index columns = HH.tr_ $ hd : tl
  where
